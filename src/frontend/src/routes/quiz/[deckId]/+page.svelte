	...
	<div class="relative z-10 container mx-auto max-w-4xl px-6 py-12 md:py-20">
			...
			<div class="space-y-12" in:fade>
				<header class="space-y-6 text-center">
					<div class="relative inline-block">
						...
						<div class="relative flex h-24 w-24 items-center justify-center rounded-[2rem] border border-primary/20 bg-card shadow-2xl">
							<BrainCircuit class="h-12 w-12 text-primary" />
						</div>
					</div>
					<h1 class="font-display text-5xl leading-tight font-black tracking-tighter uppercase md:text-7xl">
					...
					<div class="space-y-8">
						...
						<div class="space-y-4">
							<div class="flex items-center justify-between">
								<label for="num-cards" class="flex flex-col gap-1">
									...
									<span class="text-xs text-muted-foreground">How many flashcards to include in this session</span>
								</label>
							...
							/>
							<div class="flex justify-between text-[9px] font-black tracking-widest text-muted-foreground uppercase">
								<span>5</span>
								<span>10</span>
								<span>15</span>
								<span>20</span>
						...
						<div class="flex items-center justify-between rounded-2xl border border-border bg-muted/30 p-5">
							<div class="flex items-center gap-4">
								...
								<div>
									...
									<span class="text-xs text-muted-foreground">Present cards in a random order each time</span>
								</div>
							</div>
							<button
								type="button"
								...
								class="relative h-7 w-14 shrink-0 overflow-hidden rounded-full transition-colors duration-200 focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none {randomOrder ? 'bg-primary' : 'bg-muted'}"
							>
								<span class="pointer-events-none absolute top-[3px] left-[3px] h-[22px] w-[22px] rounded-full bg-background shadow-md transition-transform duration-200 {randomOrder ? 'translate-x-7' : 'translate-x-0'}"></span>
						...
						<div class="flex items-center justify-between rounded-2xl border border-border bg-muted/30 p-5">
							<div class="flex items-center gap-4">
								...
								<div>
									<span class="block font-display text-sm font-black uppercase">Include All Cards</span>
									<span class="text-xs text-muted-foreground">Study all cards now, ignoring the spaced repetition schedule</span>
								</div>
							</div>
							<button
								type="button"
								...
								class="relative h-7 w-14 shrink-0 overflow-hidden rounded-full transition-colors duration-200 focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none {ignoreInterval ? 'bg-primary' : 'bg-muted'}"
							>
								<span class="pointer-events-none absolute top-[3px] left-[3px] h-[22px] w-[22px] rounded-full bg-background shadow-md transition-transform duration-200 {ignoreInterval ? 'translate-x-7' : 'translate-x-0'}"></span>
						...
						{#if !ignoreInterval}
							<div class="flex items-start gap-3 rounded-xl border border-warning/20 bg-warning/5 p-4" in:slide>
								...
								<p class="text-xs text-warning">
									Only cards that are <strong>due for review</strong> today will appear. If no cards are due, the quiz won't start. Toggle <strong>Include All Cards</strong> to study everything regardless.
								</p>
							</div>
						{/if}
						...
						{#if error}
							<div class="flex items-start gap-3 rounded-xl border border-destructive/30 bg-destructive/10 p-4" in:slide>
		...
		{:else if isLoading}
			<div class="flex flex-col items-center justify-center space-y-8 py-32" in:fade>
				...
				<div class="space-y-2 text-center">
					<h2 class="font-display text-xl font-black tracking-widest uppercase italic">Preparing Quiz</h2>
		...
		{:else if error}
			<div class="flex flex-col items-center justify-center space-y-8 py-20 text-center" in:fade>
				...
				<div class="space-y-4">
					...
					<p class="mx-auto max-w-md text-muted-foreground">The connection to the server was interrupted. Please try again.</p>
				</div>
				<Button onclick={() => goto(`/decks/${deckId}`)} variant="outline" class="h-14 rounded-2xl border-border px-8 font-black tracking-widest uppercase">
					Return to Deck
				</Button>
			</div>
		{:else if quizFinished && quiz}
			<div class="space-y-12" in:fly={{ y: 40, duration: 800 }}>
				<header class="space-y-6 text-center">
					<div class="inline-flex items-center gap-3 rounded-full border border-success/20 bg-success/10 px-6 py-2 text-success">
				...
				<div class="grid grid-cols-1 gap-6 md:grid-cols-3">
					<div class="flex flex-col items-center justify-center space-y-2 rounded-[2.5rem] border border-border bg-card/50 p-8 text-center backdrop-blur-xl">
						<span class="text-[10px] font-black tracking-widest text-muted-foreground uppercase">Accuracy</span>
						<div class="font-display text-4xl font-black text-primary">
							{Math.round((quiz.score / totalCards) * 100)}%
						</div>
					</div>
					<div class="flex flex-col items-center justify-center space-y-2 rounded-[2.5rem] border border-border bg-card/50 p-8 text-center backdrop-blur-xl">
						<span class="text-[10px] font-black tracking-widest text-muted-foreground uppercase">Correct Answers</span>
						<div class="font-display text-4xl font-black text-success">
							{quiz.score} / {totalCards}
						</div>
					</div>
					<div class="flex flex-col items-center justify-center space-y-2 rounded-[2.5rem] border border-border bg-card/50 p-8 text-center backdrop-blur-xl">
						<span class="text-[10px] font-black tracking-widest text-muted-foreground uppercase">Time Spent</span>
				...
				<div class="flex flex-col justify-center gap-4 pt-8 sm:flex-row">
					<Button
						...
						class="h-16 rounded-2xl bg-primary px-10 font-black tracking-widest uppercase transition-transform hover:scale-105"
					>
			...
			<div class="space-y-8" in:fade>
				...
				<div class="flex items-center justify-between">
					<Button
						variant="ghost"
						onclick={() => goto(`/decks/${deckId}`)}
						class="group flex items-center gap-2 text-muted-foreground hover:text-foreground"
					>
					...
					<div class="flex items-center gap-6">
						<div class="hidden items-center gap-2 text-muted-foreground md:flex">
							<Timer class="h-4 w-4" />
							<span class="text-[10px] font-black tracking-widest uppercase tabular-nums">{formatTime(elapsedTime)}</span>
						</div>
						<div class="flex items-center gap-3">
							...
							<div class="h-1.5 w-32 overflow-hidden rounded-full bg-muted">
								<div class="h-full bg-primary transition-all duration-700 ease-out" style="width: {((currentIndex + 1) / totalCards) * 100}%"></div>
							</div>
						</div>
					</div>
				</div>
				...
				<div class="relative flex items-center justify-center gap-4 py-8">
					...
					<button
						...
						class="relative z-50 flex h-14 w-14 shrink-0 cursor-pointer items-center justify-center rounded-full border border-border bg-card text-foreground shadow-2xl transition-all hover:bg-muted disabled:cursor-not-allowed disabled:opacity-10"
					...
					<div class="relative z-10 max-w-2xl min-w-0 flex-grow">
						{#key currentIndex}
							<div in:fly={{ x: 50, duration: 250, delay: 100 }} out:fly={{ x: -50, duration: 150 }}>
								<Flashcard
									{...currentCard}
					...
					{#if pendingAnswer !== null && pendingAnswer.isCorrect && pendingRating === null}
						<div class="fixed right-0 bottom-0 left-0 z-50 flex justify-center p-6 sm:bottom-6 sm:p-0" in:fly={{ y: 50, duration: 250, opacity: 0 }}>
							<div class="w-full max-w-sm rounded-[2rem] border border-border/50 bg-card/95 p-6 shadow-2xl backdrop-blur-2xl sm:max-w-md">
								<p class="mb-4 text-center text-xs font-black tracking-widest text-muted-foreground uppercase">
									...
									{#each [1, 2, 3, 4, 5] as rating}
										<div class="flex flex-col items-center justify-start gap-3">
											<button
												...
												class="flex h-12 w-full max-w-[3.5rem] items-center justify-center rounded-xl border font-display text-lg font-black transition-all hover:-translate-y-1 hover:shadow-lg active:scale-95 {rating <= 2 ? 'border-destructive/30 bg-destructive/10 text-destructive hover:bg-destructive/20' : rating === 3 ? 'border-warning/30 bg-warning/10 text-warning hover:bg-warning/20' : 'border-success/30 bg-success/10 text-success hover:bg-success/20'}"
											>
												{rating}
											</button>
											<span class="text-center text-[7px] font-black tracking-widest text-muted-foreground uppercase sm:text-[9px]">
												{#if rating === 1}
													Again
												{:else if rating === 2}
													Hard
				...
				{#if isCurrentAnswered || currentIndex === totalCards - 1}
					<div class="mt-12 flex justify-center" in:scale={{ start: 0.5, duration: 400 }}>
						{#if currentIndex === totalCards - 1}
							<Button onclick={finishQuiz} size="lg" class="h-16 rounded-2xl bg-success px-12 font-black tracking-[0.2em] uppercase shadow-lg transition-all hover:scale-105 active:scale-95">
								Finish Quiz
								<CheckCircle2 class="ml-3 h-5 w-5" />
							</Button>
						{:else}
							<Button onclick={nextCard} size="lg" class="h-16 rounded-2xl bg-primary px-12 font-black tracking-[0.2em] uppercase shadow-lg transition-all hover:scale-105 active:scale-95">
								Next Question
